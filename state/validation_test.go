	...
	for height := int64(1); height < validationTestsStopHeight; height++ {
		...
		*/
		for _, tc := range testCases {
			block, _ := state.MakeBlock(nil, height, makeTxs(height), lastCommit, nil, proposerAddr)
	...
	for height := int64(1); height < validationTestsStopHeight; height++ {
		...
		if height > 1 {
			...
			*/
			...
			block, _ := state.MakeBlock(nil, height, makeTxs(height), wrongHeightCommit, nil, proposerAddr)
			err = blockExec.ValidateBlock(state, block)
			...
			*/
			block, _ = state.MakeBlock(nil, height, makeTxs(height), wrongSigsCommit, nil, proposerAddr)
			err = blockExec.ValidateBlock(state, block)
